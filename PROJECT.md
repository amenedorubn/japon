...
| `ruta-japon-2027.ics` / `realidad-japon-2027.ics` | Feed de calendario ESTÁTICO (12.73, Prioridad 5): generado por `tools/ics-export.js`, publicado por GitHub Pages con URL estable para suscripción (Guía → 📅 Calendario). Regenerar y comitear tras cambios en `RUTA_DAYS`/`TRANSPORT`/`FLIGHTS` (o con un volcado de Firebase en vivo para el plan real) |
...
| 12.63 | `24814bc` | **Consistencia final + noches por reservar reales** | JR Pass: corregida la inconsistencia numérica en `CHECKLIST`/docx (los 6 tramos suman ~¥58.000, MENOS que el pase de ¥80.000, no "aproximadamente igual"). `SKIPPED` gana categoría `pendiente` (Narai-juku: decisión de equipo abierta, no descarte). `CHECKLIST` gana las 3 reservas que faltaban (Tōshōgū online, Myōryū-ji por teléfono, cena de cumpleaños). Docx: mismos 3 fixes + frase de sakura tardío del 26-abr suavizada. **"Aún por reservar" reescrito**: usaba 3 placeholders fijos `hotel_tokyo/kyoto/osaka` de la semilla vieja (obsoletos desde la Ruta); ahora sale de `NIGHTS` (filas `res`/`amp`, con check-in/check-out ISO y nº de noches añadidos como campos 4-6) — dice ciudad + fechas reales + noches, no solo "por reservar" |
...
| 12.71 | `(este commit)` | **Booking Timeline (Prioridad 3, fase de viaje): "¿qué toca reservar A CONTINUACIÓN?"** | Nueva sección en Guía (`#bookingTimeline`, cabecera de la pestaña) + teaser en Inicio (`#bookingTeaser`, junto a "Aún por reservar") que responden la pregunta con datos YA documentados en `CHECKLIST`/`TRANSPORT`/`NIGHTS` (ninguna fecha de venta inventada). `BOOKINGS` (nuevo, 10 entradas: Shibuya Sky, Tōshōgū, bus Nouhi, cena de cumpleaños, ryokan de Takayama/Nikko, Myōryū-ji, bus Kawaguchiko, USJ, teamLab) trae, por reserva, la fecha en que hace falta EN JAPÓN + `opensDaysBefore` (regla de apertura real si se conoce, `null` si no hay venta programada = "reservable ya") + `pid` (reutiliza la web del catálogo, sin duplicar URL) o `url` cuando no hay ficha propia. `bookingWindow(b, today)` es pura (recibe `today`, nunca `Date.now()` implícito: se puede fijar en tests) y calcula si la venta ya abrió. `bookingRows()` fusiona `BOOKINGS` con las noches `res`/`amp` de `NIGHTS` (mismo filtro que `renderHoteles`/`renderInicio`, sin duplicar esa lista) — las noches, al no tener venta programada, caen siempre en "reservable ya". La vista separa dos cubos: "📋 Puedes reservar YA" (ordenado por la fecha del viaje que cubre, más urgente primero) y "⏳ Aún no ha abierto la venta" (ordenado por cuándo abre, con cuenta atrás en días). **Hallazgo sin corregir esta sesión**: `CHECKLIST` sigue mencionando "Reservar Jambo Hanare y Kitan Hibiki", dos restaurantes que solo existen en `SEED_DAYS` (la Propuesta retirada de la UI en 12.63) y NO están en `RUTA_DAYS`; quedan fuera del Booking Timeline a propósito (no hay fecha de viaje real que asignarles) — decisión del usuario pendiente sobre si siguen en el plan |
| 12.70 | `(este commit)` | **Agenda 2.0 (Prioridad 2, fase de viaje): origen/destino visibles + "(estimado)" explícito** | Cada tramo de transporte de Realidad nombra ahora origen → destino como texto (`.t-route`, antes solo vivía dentro del enlace de Google Maps). `realGapWindow` gana el campo `estimated`: el colchón de 20 min de pre/post (una convención, no un dato) se marca así; el hueco entre dos paradas reales NO, porque las dos horas ya las fijó el usuario. `transferHTML` también marca "(estimado)" cuando la opción de transporte seleccionada es un paseo auto-inferido sin confirmar (`opt.auto`, ya existía como dato del modelo — Prioridad 3 solo lo hace VISIBLE). El .ics de Realidad (`realDayTimelineEntries`/`realidadICS`) hereda lo mismo: los tramos de transporte llevan origen/destino en el SUMMARY y "(estimado)" en la DESCRIPTION cuando aplica — la Ruta (`dayTimeline`/`rutaICS`, 12.66-12.67) ya lo tenía. Duración y tarifa por tramo ya vivían en los chips de transporte (`transferOptionChip`) desde antes de esta fase; reserva obligatoria, desde 12.68 |
| 12.69 | `(este commit)` | **Mapa: un solo lenguaje visual de marcador** | `pinIcon(tag, kind, content, color, size, ring)`, fábrica ÚNICA de icono para TODO el mapa: círculo con contenido (número o emoji) y 3 variantes de aro que se leen sin texto — `primary` (aro blanco sólido, o el color propio del itinerario vía `ITIN_RING`: parada real/POI del plan), `optional` (aro discontinuo, más pequeño: extra "si sobra tiempo"), `structural` (aro reforzado, color fijo: aeropuerto/vuelo). Antes cada tipo se pintaba a mano con su propio criterio (número plano en rojo fijo, punto SVG de color sin icono, círculo de aeropuerto sin clase compartida, avión suelto sin círculo) — cuatro lenguajes distintos en la misma vista. Los POIs de "Todos" pasan de punto de color a círculo con el EMOJI de categoría (mismo color que la parada numerada del día: "de un vistazo, qué es esto" sin abrir la ficha). La leyenda del mapa (`#mapLegend`) explica el lenguaje de marcador con un día concreto seleccionado, antes de los tramos de transporte. Los modos de transporte ya tenían color+trazo distintos por modo (a pie/metro/tren/bus/ferry, con leyenda) desde antes de esta fase; no se tocan. Tests actualizados en `test-8a-map.js` (los recuentos de POI que dependían del tipo de capa Leaflet `circleMarker` ahora filtran por el tag de `className`, ya que todo pasa a ser capa `marker`) + 3 checks nuevos de lenguaje visual |
| 12.68 | `(este commit)` | **Fase de viaje: las 5 prioridades del acompañante durante el viaje** | Con la planificación dada por completa, arranca la fase de "mejor compañero durante el viaje real": **P1** "〰️ Líneas Dani" ahora solo se muestra viendo el itinerario de Dani (`setItinMode` oculta/enseña el chip; antes vivía siempre visible). **P2** los extras "si sobra tiempo" se leen ahora COMO PARTE del recorrido de un día concreto (`insertExtrasInline`, clase `.opt-stop`, borde punteado) tanto en Ruta como en Realidad, en vez de en una sección aparte al final; en el mapa, con un día elegido, cada extra dibuja una rama punteada (`offsetLatLng` + polilínea) hacia un pin separado del punto real, en vez de solaparse con él — "Todos los días" conserva el `<details>` colapsado de siempre (sin cambios). **P3** Realidad gana agenda de verdad: cada hueco de transporte muestra su hora inferida (`realGapWindow`/`stopWindow`, derivada de las horas que ya escribe el usuario en las paradas, igual que `dayTimeline` para la Ruta) y un aviso 🎫 si el texto/nota del trayecto menciona una reserva. **P4** nuevo botón "📅 Exportar a calendario" en Realidad (`realidadICS`/`downloadRealidadICS`): a diferencia del .ics de la Ruta (referencia fija), este recorre `state.days` en vivo — cada edición del plan real se refleja en el siguiente export, sin datos duplicados. **P5** indicador de intensidad por día (`intensityScore`/`INTENSITY_LEVELS`: 🟢 Relajado · 🟡 Moderado · 🟠 Ajetreado · 🔴 Muy intenso), calculado sobre datos YA existentes (nº de paradas, minutos de actividad, km caminados entre paradas contiguas, cambios de medio de transporte, tramo real entre ciudades); visible en el Cord (de un vistazo), en la cabecera del día de Realidad y en cada tarjeta de la Ruta. Umbrales calibrados a ojo contra los 18 días con contenido de la Ruta (reparto ~5/6/4/3). Test nuevo `test-12-travel-agenda.js` (21 checks) cubre las 5 prioridades |
...
hotels · confirmed · axis · cord · seasons · realidad · zones · ruta · travel-agenda (las 5
prioridades de la fase de viaje, 12.68). (`test-7c-lists.js` se retiró
...
26-abr en el docx). *(12.63: ítems 1, 5, 6 y 8 RESUELTOS — ver más abajo. Ítem 7 queda deliberadamente
sin tocar: no es una inconsistencia sino una posible mejora de detalle, y darle ficha propia a
Katamachi/Ebisu Yokochō exigiría 2 lugares de catálogo nuevos + tocar el recuento de paradas de
`test-12-ruta.js`, coste desproporcionado para lo que aporta.)*
...
## 15. Arquitectura preparada para un futuro "modo en vivo" (Prioridad 6, fase de viaje — NO implementado)

Petición explícita del usuario: preparar la arquitectura para un futuro modo inspirado en Dynamic
Island/Live Alerts (qué toca hacer ahora, próximo destino, tiempo de trayecto restante, próxima
reserva, retrasos, cuentas atrás) **sin implementar la UI todavía**. Lo que existe hoy (12.71):

- **`nowStatus(entries, nowMin)`** (`index.html`, junto a `realDayTimelineEntries`): función PURA
  que recibe las entradas YA derivadas por `realDayTimelineEntries(d)` — los mismos datos que
  alimentan el render editable de Realidad y su export `.ics`, ninguna fuente nueva — más la hora
  actual en minutos, y devuelve `{current, next, upcomingReservation}` con los minutos restantes/
  hasta que empiece cada una. No pinta nada, no engancha ningún timer: la futura UI solo tendría
  que llamarla cada minuto con la hora real y leer los campos ya calculados.
- Las entradas de `realDayTimelineEntries` ganaron dos campos (sin tocar el modelo persistido:
  son datos DERIVADOS, no `state`): `pid` en las actividades (para abrir la ficha del sitio "de lo
  que toca ahora" sin volver a buscarlo) y `reservation` en los tramos de transporte (mismo
  criterio que `transferHTML`/`needsReserve`, para "próxima reserva" sin re-derivar el texto).
- Verificado con `test-12-travel-agenda.js` (Prioridad 6, 6 checks): entrada en curso, ninguna
  entrada inventada antes/después del día, próxima reserva encontrada y descartada al pasar.

**Deliberadamente fuera de alcance (documentado, no resuelto):**
- **Retrasos en vivo**: exigiría una fuente externa de tránsito en tiempo real (GTFS-realtime o
  equivalente de las operadoras japonesas), que rompería el patrón "sin dependencias de runtime
  externas para lo que la app necesita para funcionar" (§13). Si se decide perseguir, es una
  conversación aparte con el usuario, no una extensión silenciosa de `nowStatus`.
- **Notificaciones/UI en vivo**: exigirían el Service Worker despierto en segundo plano (hoy
  `sw.js` solo cachea assets, cache-first, sin lógica en segundo plano) y una superficie visual
  nueva (widget/Live Activity/Dynamic Island real requieren app nativa o PWA con capacidades que
  este proyecto no tiene hoy). Ninguna de las dos se ha tocado.
- **Modelo persistido**: sin cambios. `state.days`/`RUTA_DAYS` no ganan campos nuevos; todo lo que
  `nowStatus` necesita ya vivía en las paradas/trayectos (horas, notas, opción de transporte
  elegida) o se deriva sin persistir (`pid`/`reservation` en las entradas, no en `state`).

Camino para cuando se decida implementar la UI: una pantalla o banner que llame `nowStatus` sobre
`realDayTimelineEntries(state.days[curDay])` con la hora real (`setInterval`, como ya hace
